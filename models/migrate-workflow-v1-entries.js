...
export async function processEntriesInBatch(args) {
  ...
  if (totalItemsProcessed === 0) {
    ...
    info(
      `- to "${chalk.underline(
        workflowDefinition.steps[stepId].name
      )}" of workflow "${chalk.italic(workflowDefinition.name)}"`,
      8
    );
    ...
    shouldRemoveTagsFromEntries =
      shouldRemoveTagsFromEntries ?? shouldCleanUpTags;
    if (shouldRemoveTagsFromEntries === undefined) {
      const answerRemoveTagMapping = await inquirer.prompt({
        ...
        message: `${INDENT.repeat(
          6
        )}Do you want to remove the tag from entries after migration?`,
      });
      shouldRemoveTagsFromEntries =
        answerRemoveTagMapping.shouldRemoveTagsFromEntries;
    }
  ...
  for (const entry of entries.items) {
    ...
    if (
      ...
      warning(
        `${CROSS} ${entry.sys.id} - Entry content type '${entry.sys.contentType.sys.id}' not configured for new workflow '${workflowDefinition.name}'`,
        6
      );
      continue;
    }
    if (!dryRun) {
      ...
      try {
        ...
        if (e.message.includes("an active workflow already exists")) {
          warning(
            `${CROSS} ${entry.sys.id} - A workflow already exists for this entry`,
            6
          );
        } else {
          error(
            `${CROSS} ${
              entry.sys.id
            } - could not create workflow for entry. Reason ${
              e.message ?? "Unknown"
            }`,
            6
          );
